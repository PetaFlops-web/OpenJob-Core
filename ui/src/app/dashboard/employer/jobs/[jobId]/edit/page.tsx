...
export default function EditJobPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const job = jobRes.data as Job & { company?: { id: string }; category?: { id: string } }
        setTitle(job.title ?? '')
        setDescription(job.description ?? '')
        setCategoryId(job.category_id ?? catList[0]?.id ?? '')
        ...
        setIsSalaryVisible(job.is_salary_visible ?? true)
        setRequirements(parseStringArray(job.requirements))
        setBenefits(parseStringArray(job.benefits))
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await jobsApi.update(jobId, {
        ...
        location_type: locationType as Job['location_type'],
        location_city: locationCity.trim() || null,
        requirements,
        benefits,
      })
      setSuccess(t('employer.jobs.updated'))
      toast.success(t('employer.jobs.updated'))
      setTimeout(() => router.push('/dashboard/employer/jobs'), 1200)
    } catch (err) {
      setError(err instanceof Error ? err.message : t('employer.jobs.updateFailed'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-4xl space-y-6 pb-12 md:space-y-8">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-semibold leading-7 text-primary">{t('employer.editJob')}</h1>
      </div>
      ...
      {success && (
        <div className="rounded-lg border border-green-300 bg-green-50 px-4 py-3 text-sm text-green-700">{success}</div>
      )}
      ...
      <section className="rounded-xl border border-border-subtle bg-surface p-4 shadow-sm md:p-6">
        <h2 className="mb-6 text-base font-semibold leading-6 text-primary">{t('employer.table.status')}</h2>
        <Select value={status} onChange={(e) => setStatus(e.target.value)}>
          <option value="draft">{t('employer.jobs.draft')}</option>
          <option value="open">{t('employer.jobs.openStatus')}</option>
          <option value="closed">{t('employer.jobs.closedStatus')}</option>
        </Select>
      </section>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        <Link
          ...
          className="rounded-lg border border-border-subtle px-6 py-3 text-center text-sm font-semibold text-on-surface-variant transition-colors hover:bg-surface-variant"
        >
          {t('common.cancel')}
        </Link>
        <button
          type="submit"
          disabled={submitting}
          className="rounded-lg bg-primary px-6 py-3 text-sm font-semibold text-on-primary shadow-sm transition-colors hover:bg-primary-container disabled:opacity-50"
        >
          {submitting ? t('employer.jobs.saving') : t('employer.jobs.saveChanges')}
        </button>
      </div>
    </form>
  )
